            ...
            <tr v-for="exchange in exchanges" :key="exchange.id">
              ...
              <td data-label="Ações" class="actions-col">
                <div class="action-buttons">
                  ...
                  <button @click="cloneExchange(exchange)" class="icon-btn clone-btn" title="Clonar">
    ...
    <div v-if="isModalOpen" class="modal-overlay" @click.self="closeModal">
      <div class="modal-content glass-card form-modal">
        <div class="modal-header">
          <h3>{{ modalMode === 'edit' ? 'Editar Exchange' : modalMode === 'clone' ? 'Clonar Exchange' : 'Nova Exchange' }}</h3>
        ...
        <form @submit.prevent="saveExchange" class="modern-form">
          ...
          <div class="modal-footer">
            ...
            <button type="submit" class="btn primary-btn" :disabled="saving">
              {{ saving ? 'Salvando...' : modalMode === 'edit' ? 'Salvar Alteracoes' : modalMode === 'clone' ? 'Salvar como Nova' : 'Salvar Exchange' }}
            </button>
          </div>
        </form>
      </div>
...
const mapExchangeToForm = (exchange) => ({
  nome: exchange.nome,
...
const openModal = (exchange = null) => {
  ...
  if (exchange) {
    ...
    form.value = mapExchangeToForm(exchange)
  } else {
...
const saveExchange = async () => {
  ...
  try {
    ...
    const successMessage = editingId.value
      ? 'Exchange atualizada com sucesso!'
      : modalMode.value === 'clone'
        ? 'Exchange clonada e criada com sucesso!'
        : 'Exchange criada com sucesso!'
