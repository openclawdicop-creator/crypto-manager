  ...
  <div class="ativos-container">
    ...
    <div class="glass-card">
      <div v-if="loading" class="loading-state">
        <span class="spinner"></span> Carregando ativos...
      </div>
        ...
        <table class="modern-table">
          ...
          <tbody>
            <tr v-if="ativos.length === 0">
              <td colspan="4" class="empty-state">Nenhum ativo cadastrado. Clique em "Novo Ativo" para adicionar.</td>
            </tr>
            <tr v-for="ativo in ativos" :key="ativo.id">
              <td data-label="ID"><strong>#{{ ativo.id }}</strong></td>
              <td data-label="Nome">{{ ativo.nome }}</td>
              <td data-label="Símbolo"><strong style="color:#0ea5e9; background:#e0f2fe; padding:2px 8px; border-radius:4px;">{{ ativo.simbolo }}</strong></td>
      ...
      <div class="modal-content glass-card form-modal">
        ...
        <form @submit.prevent="saveAtivo" class="modern-form">
          <div class="form-group">
            ...
            <input v-model="form.nome" type="text" placeholder="Ex: Bitcoin" required maxlength="100"/>
          </div>
          <div class="form-group">
            <label>Símbolo (Ticker) *</label>
            <input v-model="form.simbolo" type="text" placeholder="Ex: BTC" required maxlength="10" style="text-transform: uppercase;" @input="form.simbolo = $event.target.value.toUpperCase()"/>
          </div>
    ...
    <div v-if="isDeleteModalOpen" class="modal-overlay" @click.self="closeDeleteModal">
      <div class="modal-content glass-card form-modal">
        ...
        <div style="margin-bottom: 1.5rem; color: #334155;">
          Tem certeza que deseja excluir o ativo <strong>"{{ itemToDelete?.nome }}"</strong>?
...
const loadAtivos = async () => {
  loading.value = true
  try {
    const res = await apiFetch('/api/ativos')
    if (res && res.data) {
      ativos.value = res.data
    } else {
      ativos.value = []
    }
  } catch (error) {
    showAlert(error.message || 'Erro ao carregar ativos', 'error')
  } finally {
    loading.value = false
  }
}
...
const saveAtivo = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      url = `/api/ativos/${editingId.value}`
      method = 'PUT'
    }
    ...
    showAlert(error.message || 'Erro ao salvar ativo', 'error')
  } finally {
    saving.value = false
  }
}

const confirmDelete = (ativo) => {
  itemToDelete.value = ativo
...
const executeDelete = async () => {
  ...
  try {
    const res = await apiFetch(`/api/ativos/${itemToDelete.value.id}`, { method: 'DELETE' })
    ...
    showAlert(error.message || 'Erro ao excluir ativo.', 'error')
    closeDeleteModal()
  }
}
